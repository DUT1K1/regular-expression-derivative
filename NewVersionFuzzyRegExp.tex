...
\begin{definition}[Similarity on Regular Expressions - G\"odel's T-norm] \label{def:Rexp}Given a similarity relation $\mathcal R$ on the alphabet, define a proximity on regular expressions
 $\mathcal{G}^{E}:E \times E \rightarrow [0,1]$ is defined as follows
...
If we consider arbitrary T-norms, Definition~\ref{def:Rexp} is too generous over regular expressions. If $\mathcal R(a,b)\lneq 1$ then $\mathcal R^L(\mathcal L(a^\ast),\mathcal L(b^\ast))\lneq \mathcal R(a,b)$ unless $R(a,b)$ is idempotent. But a norm where all values are idempotent is necessarily G\"odel's minimum T-norm. 
...
$$\mathcal{R}^E(r,s) = \begin{cases}1 & \mbox{ if } r=s; \\
                                                       ...
                                                       \otimes \mathcal{R}^E(r_1,s_1)+\mathcal{R}^E(r_2,s_2) & \mbox{ if } r=r_1+r_2 \mbox{ and } s=s_1+s_2; \\
                                                       \otimes \mathcal{R}^E(r_1,s_1)\cdot\mathcal{R}^E(r_2,s_2) & \mbox{ if } r=r_1\cdot r_2 \mbox{ and } s=s_1\cdot s_2; \\
