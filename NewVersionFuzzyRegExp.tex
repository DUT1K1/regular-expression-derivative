...
\title{Fuzzifying Regular Expressions}
\author{
...
\item {\em Restricted Regular expressions}, $R^\Sigma$ are the regular expressions over $\Sigma$ such in which the constructors $\&$ and $\neg $ do not appear.
\end{enumerate}
\end{definition}
...
\]
\end{definition}
\paragraph{Notation} Regular expressions, possibly restricted, are denoted by $r,s$ possibly with indices and apexes.  We will write $r[a_1,\ldots,a_n]$ in order to highlight the occurrences $a_1,\ldots,a_n$ of alphabet symbols  in $r$. By $r[s]$ we denote the regular expression obtained by replacing $s$ in place of $a$ in $r[a]$.
...
There are various equational systems which ensure semantic equivalence between regular expressions. In this paper we consider the one by Brzozowski see~\cite{B62} and its extension by Coquand--Siles~\cite{CoquandRegexEquality}, see Definition~\ref{def:similarity}. It is well-known that there is no purely equational finite characterization, see~\cite{Salomaa,Kozen}. This derives from the existence of the $\ast$ operator which equationally can only be finitely approximated. Examples of semantic equivalent regular expressions which are not similar, in the sense of Coquand  and Siles are $a^\ast$ and $(\epsilon+a)^\ast$. Brzozowski results are significant in that  even if there is no finite equational axiomatization of semantic equivalence, nevertheless a finite approximation of  equality is enough to achieve  finiteness in derivatives and hence in states.  
\end{remark}

\begin{definition}[Similarity ($\sim$)\cite{CoquandRegexEquality}]\label{def:similarity}
...
and extended with additional simplifying laws (e.g.\ $r+\emptyset\sim r$, $\neg\neg r\sim r$, etc.). {\color{red} Please give the full list}
\end{definition}
...
\begin{lemma}[Canonicalization]\label{def:canonize} The equations in Definition~\ref{def:similarity} can be used to define a 
...
Given a regular expression $r[a]$ by $r[a]^\#$ we denote the formal expression such that $L(r[a]^\#)= \bigcup_{n \geq 0} \mathcal{L}(r[a]^\#_n)$ 
   where $ \mathcal L(r[a]_{n+1})=\mathcal L(r[r[a]_n])${ and }$\mathcal{L}(r[a]_0)=  \mathcal L(r[a]).$
\end{definition}

\begin{proposition}
The constructors in Definition~\ref{extension} generate regular languages.
\end{proposition}
...
\begin{definition}[Similarity on Regular Expressions] \label{def:Rexp}Given a similarity relation $\mathcal R$ on the alphabet, define a similarity o regular expressions
...
\item We can incorporate in the above definition finite equational properties among regular expressions, but no exhaustive finitary definition can be given. For example we can put $\mathcal R^e(e_1,d_1) = \mathcal R^e(e_1,d_2)$ if ${\sf nf}(d_1)={\sf nf}(d_2)$. we shall not pursue this.
\item 
...
\[\mathcal L^\mu (r)=\{w\mid \mathcal w\sim_\mu^{\Sigma^\ast}v)\mbox{ for } u\in \mathcal L(r)\}.\]
\end{definition}
