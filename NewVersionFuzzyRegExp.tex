...
{\color{red} CAVEAT For an arbitrary tensor this notion is too strict. If $\mathcal R(a,b)\leq 1$ then $\mathcal R(a^\ast,b\ast)$ is usually 0 for a generic T-norm, \eg\ Gougen's product T-norm. Of course we can use some speial combination of Godel and Gougen such as 

$T(x,y)=\begin{cases} \min(x,y) & x,y>p\\ x\times y & \mbox{ otherwise. }
\end{cases}$

Thus many of the results in the paper \eg Lemma\ref{thm:syntax-semantics} fail for a  general T-norm.}
