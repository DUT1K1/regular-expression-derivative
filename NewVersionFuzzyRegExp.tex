...
%\begin{theorem}\label{thm:Rr-RL-bridge}For all restricted regular expressions $r$ and $s$, the value $\mathcal{R}^L\bigl(L(r),L(s)\bigr)$ is given by:
%\[\sup\Bigl\{\min\bigl(\mathcal{R}^E(r',p),\mathcal{R}^E(s',q)\bigr)\;\Bigm|\; r',s',p,q \in R^\Sigma,\; L(r')=L(r),\;L(s')=L(s),\; L(p)=L(q)\Bigr\}.\]\end{theorem}
...
%\item Lemma~\ref{thm:syntax-semantics}  and Theorem~\ref{thm:Rr-RL-bridge} are not true for  regular expressions when $\wedge$ and $\neg$ are allowed.
