...
We define similarity between symbols to be $\mathcal{R}:\Sigma \times \Sigma \rightarrow [0,1]$ and based on that we can define similarity between  regular expressions and words as follows. If we take G\"odel's T-norm definitions simplify considerably.
...
\end{cases}$$
\end{definition}
Viewing words as regular expressions, the above definition naturally extends to regular expressions if we take G\"odel's T-norm, .
...
$$\mathcal R^{E}(r_1,r_2) =\begin{cases}\bigotimes_{1\leq i\leq n}\mathcal R(a_i,b_i) &\mbox{ if } r_1=r[a_1,\ldots,a_n]\mbox{ and }r_2 =r[b_1,\ldots,b_n]
\\ 
0& \mbox{  otherwise }
\end{cases}$$
\end{definition}
...
\]
\end{definition}
\begin{remark} \label{general-norms} Definition~\ref{def:Rexp} of similarity of regular expressions when considering  arbitrary T-norms is too generous. Namely if $\mathcal R(a,b)\lneq 1$ then $\mathcal R^L(\mathcal L(a^\ast),\mathcal L(b^\ast))\lneq \mathcal R(a,b)$ unless $R(a,b)$ is idempotent. But a norm where all values are idempotent is necessarily G\"odel's minimum T-norm. 
\\ Nevertheless there are many non-idempotent T-norms, \eg\ 
$$T(x,y)=\begin{cases}p+ \dfrac{(x-p)(y-p)}{1-p} & x,y\geq p\\ \min(x,y) & \mbox{ otherwise }
\end{cases}$$, where $p\in [0,1]$
which we do want to consider. Thus Definition~\ref{def:Rexp} needs to be amended as follows
\end{remark}

\begin{definition}[Similarity on Regular Expressions - General T-norm] \label{def:Rexp}
...
$$\mathcal{R}^e(r,s) = \begin{cases}\mathcal{R}(a,b)  & \mbox{ if } r=a \mbox{ and } s=b; \\
                                                       ...
                                                       \bigvee_n \otimes^n \mathcal{R}^e(r_1,s_1)  & \mbox{ if } r=r_1^\ast \mbox{ and } s=s_1^\ast; \\
                                                        0 & \mbox{ otherwise.}
\end{cases}$$
\end{definition}
