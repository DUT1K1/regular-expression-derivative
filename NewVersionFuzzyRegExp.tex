...
{\bf An side on substitution} \label{extension}
...
$$\mathcal R^{E}(r_1,r_2) =\begin{cases}\bigotimes_{1\leq i\leq n}\mathcal R(a_i,b_i) &{\color{red} \mbox{ without repeating equal pairs } (a_i,b_i)}\\ & \mbox{ if } r_1=r[a_1,\ldots,a_n] \mbox{ and }r_2 =r[b_1,\ldots,b_n]
\\ 
0& \mbox{  otherwise }
\end{cases}$$
{\color{red}I we take as T-norm  G\"odel's T-norm, the non-repetitions proviso can be dropped.}
\end{definition}
...
{\color{red} \begin{remark} The reason for introducing the non-repeating condition for arbitrary T-norms arises because if we take the unrestricted definition we have that  $\mathcal R(a,b)\leq 1$ then $\mathcal R^L(\mathcal L(a^\ast),\mathcal L(b^\ast))$ is 0, \eg\ Gougen's product T-norm. Thus many of the results in the paper \eg Lemma\ref{thm:syntax-semantics} fail for a  general T-norm. Of course in some special cases, \eg\ if we  use some specific combination of G\"odel's  and Gougen's T-norm such as 

$T(x,y)=\begin{cases} \min(x,y) & x,y>p\\ x\times y & \mbox{ otherwise }
\end{cases}$

the proviso can be dropped and the theorems can be partially recovered. 
\end{remark}}
