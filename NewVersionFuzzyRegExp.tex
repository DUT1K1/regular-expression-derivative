...
\begin{definition}[Similarity on Words]\label{def:Rw}Given a similarity relation $\mathcal R$ on $\Sigma$, define a proximity relation on  words $\mathcal{R}^w:\Sigma^* \times \Sigma^* \rightarrow [0,1]$ as follows:$$\mathcal R^{E}(w_1,w_2) =\begin{cases}\bigotimes_{1\leq i\leq n}\mathcal R(a_i,b_i) &\mbox{ if } w_1=a_1\ldots a_n\mbox{ and }w_2 =b_1 \ldots b_n
\\ 
0& \mbox{  otherwise }
\end{cases}$$
\end{definition}
\begin{definition}[Similarity on languages]\label{def:RL}
For languages $L_1,L_2\subseteq \Sigma^*$ define a procxmity relation $\mathcal{R}^L\subseteq \mathcal P(\Sigma^\ast)\times \mathcal P(\Sigma^\ast)$ as follows
\[
\mathcal{R}^L(L_1,L_2)\;=\;
\min\!\Bigl\{
...
$$\mathcal{R}^e(r,s) = \begin{cases}\mathcal{R}(a,b)  & \mbox{ if } r=a \mbox{ and } s=b; \\
