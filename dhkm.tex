...
There is growing interest in the computer science and AI communities in moving from {\em exact}, {\em crisp}, {\em qualitative} settings to {\em metric}, {\em probabilistic}, {\em quantitative}~\cite{AlvesKesnerRamos2025,SilvaBonchiBonsangueRutten2011} and hence ultimately {\em fuzzy} settings~\cite{met05}. In this view, we explore how to {\em fuzzify} the standard theory of {\em regular languages}. Traditionally this has been done fuzzifying the theory of finite automata~\cite{C71}, by turning membership functions into lattice-ordered monoids, see \eg\cite{LP06}. Our approach is different in that we relax the crisp setting based on exact equality of symbols, to a setting where symbol matching is fuzzy, as was originally considered in~\cite{Santos68,WeeFu69}. Namely, following similarity-based reasoning~\cite{Sessa2002},
...
\paragraph{Notation} Regular expressions, possibly restricted, are denoted by $r,s$, possibly with indices or superscripts. We write $r[a_1,\ldots,a_n]$ to highlight the occurrences $a_1,\ldots,a_n$ of alphabet symbols in $r$. By $r[s_1,\ldots,s_n]$ we denote the regular expression obtained by positionally replacing $a_1,\ldots,a_n$ with $s_1,\ldots,s_n$ in $r[a_1,\ldots,a_n]$. We write $r\equiv s$, and say that $r$ and $s$ are \emph{semantically equivalent}, when $\mathcal L(r)=\mathcal L(s)$.\smallskip


 \begin{remark}{\bf On the Algebra of Regular Events}
There are various equational systems that ensure semantic equivalence between regular expressions. In this paper we consider Brzozowski's system~\cite{B62} and its extension by Coquand--Siles~\cite{CoquandRegexEquality}; see Definition~\ref{def:similarity}. It is well known that there is no finite purely equational characterization of semantic equivalence~\cite{Redko}; the complete systems of~\cite{Salomaa,Kozen} are not purely equational. This is due to the $\ast$ operator, which can only be finitely approximated equationally. Examples of semantically equivalent regular expressions that are not similar, in the sense of Coquand and Siles, are $a^*$ and $(\varepsilon+a)^*$. Brzozowski's results are significant because, even though there is no finite equational axiomatization of semantic equivalence, a finite approximation of equality is enough to achieve finiteness in derivatives and hence in states.
\end{remark}
...
\end{array}
\]
Here $\top$ abbreviates $\neg\emptyset$. We take $\sim$ to be the least congruence containing these equations, so it may be applied inside subexpressions.
\end{definition}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Fuzzy Relations and T-Similarity}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

In this paper we use standard notions in fuzzy logic, see~Appendix~\ref{appendix} or \cite{met05,Sessa2002} for more details. Throughout the paper, $\otimes$ denotes a left-continuous T-norm, see Definition~\ref{T-norm}, on the real interval $[0,1]$. We write it in infix
notation. 
...
\item a fuzzy relation $\cR$ is a \emph{T-similarity relation}, with respect to a T-norm, $\otimes$, if for all $s,s_1,s_2$ it is {\em reflexive}, namely $\cR(s,s)=1$, {\em symmetric}, namely $\cR(s_1,s_2)=\cR(s_2,s_1)$, and satisfies the {\em reverse
triangle inequality} with respect to $\otimes$, namely $\cR(s_1,s_2)\ge \cR(s_1,s) \otimes \cR(s,s_2)$;
...
\begin{enumerate}
\item The same fuzzy relation can be a T-similarity relation w.r.t. different norms; trivial examples are crisp equivalence relations. 
...
\subsection{T-similarities on Words, Regular Expressions, and Languages}
...
\begin{definition}[T-similarity on Regular Expressions for G\"odel's T-norm] \label{def:Rexp}Let T be G\"odel's T-norm. Given a T-similarity relation $\mathcal R$ on $\Sigma$, define a fuzzy relation on regular expressions, $\mathcal{G}^{E}:E \times E \rightarrow [0,1]$, as follows
...
Let $r,s$ be restricted regular expressions over $\Sigma$, and let $\mathcal{R}$ be a T-similarity relation on $\Sigma$. Define inductively the following fuzzy relation
...
{\color{red}\begin{conjecture} Let $r,s$ be restricted regular expressions over $\Sigma$, and let $\mathcal{R}$ be a T-similarity relation on $\Sigma$, then for all restricted regular expressions $r_1,r_2$ if $\mathcal{R}^E(r_1,r_2)\gneq 0$ then $r_1=r[a_1,\ldots,a_n] $ and $r_2=r[b_1,\ldots,b_n] $ for suitable choices of $r$ and $a_i$'s and $b_i$'s. 
\end{conjecture}}
...
\end{enumerate}
Moreover if $\cR$ is a G\"odelian T-similarity relation, or $\mu\otimes\mu=\mu $, then $\mathcal R^E(r_1,r_2)\geq \mu$ then $L^\mu(r_1)=L^\mu(r_2)$;
\end{theorem}
{\color{red} Rocq proof ??}\vspace{0.5cm}
...
\end{enumerate}
We could incorporate in Definitions~\ref{def:Rexp} and~\ref{def:Rexpg} finite equational relations among regular expressions, see \eg Lemma~\ref{lem:similarity-sound} or we could put $\mathcal R^E(r_1,s) = \mathcal R^E(r_2,s)$ if ${\sf nf}(r_1)={\sf nf}(r_2)$. But since no finite purely equational characterization can be given, see~\cite{Redko,Salomaa,Kozen}, we shall not pursue this.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In this section we fuzzify, w.r.t. a cut value $0<\mu\leq 1$ the definitions of syntactic and semantic derivatives. Our goal are fuzzy versions of so as to achieve the fuzzy version of Theorem~\ref{the:derivative}, namely Theorem~\ref{the:fuzzyderivative} and 
...
\{w \in \Sigma^* \mid
  ...
  :\;
  \cR(a,b)\otimes \mathcal{R}^w(w,v)\ge\cut,\ bv \in {L}
\}.
$$
\end{definition}
...
\begin{definition}\label{def:fuzzy-word-derivative}
    ...
    $$
    \partial^\cut_\varepsilon(\mathcal{L}) = \mathcal{L}^\cut,
    \qquad
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
This section is a generalization of the results in~\cite{KM26} which were originally given for G\"odel's minimum T-norm, see Definition~\ref{norms}. 
...
{\color{red} Rocq?? Proofs??}\vspace{1 cm}

{\color{red}Notice that we have also the following: 
\begin{proposition}\label{prop:cut-equivalence-notice}
...
Let $\cR$ be a g\"odelian T-similarity relation on $\Sigma$ or just a T-similarity relation for a cut value $0 < \cut \le 1$ such that $\mu\otimes\mu=\mu$, moreover let $\mathcal{L}\subseteq\Sigma^*$ and $a\in\Sigma$. 
\begin{enumerate}
...
$\cut$, $\mathcal D_a^\cut(r)$ in Definition~\ref{def:regex-fuzzy-derivative}, is equal to
...
\{w \in \Sigma^* \mid
  \exists b \in \Sigma,\ \exists v\in\Sigma^*
  :\;
  (a,b)\in\cR_\cut,\ w\sim_\cut^{\Sigma^*}v,\ bv\in\mathcal{L}
\}.
$$
\end{enumerate}
\end{proposition}
{\color{red} Rocq?? Proofs??}

Proposition~\ref{the:quotient-fuzzy-derivative} below provides the explanation for all the above simplifications, by showing that when $\cR^\mu$ is an equivalence because either $\cR$ is a g\"odelian T-similarity relation on $\Sigma$ or just a T-similarity relation for a cut value $0<\cut \le 1$ such that $\mu\otimes\mu=\mu$ then the fuzzy derivatives become ordinary derivatives if we pass to the quotient alphabet, {\em i.e.}, once letters are identified up to
...
\]
\item Extend $q_\cut$ to regular expressions $r[a_1,\ldots,a_n]$ (notation as in Section~\ref{basic}) by
\[
...
$q_\cut^*$ does exactly the same thing for a whole word, letter by letter. We can now prove the following proposition.



\begin{proposition}\label{the:quotient-fuzzy-derivative}
Let $\cR$ be a g\"odelian T-similarity relation on $\Sigma$ or just a T-similarity relation for a cut value $0 < \cut \le 1$ such that $\mu\otimes\mu=\mu$. For every language $L \subseteq \Sigma^*$ and every
$a\in\Sigma$,
\[
  \partial_a^\cut( L)
  \;=\;
...
Let $\cR$ be a g\"odelian T-similarity relation on $\Sigma$ or just a T-similarity relation for a cut value $0 < \cut \le 1$ such that $\mu\otimes\mu=\mu$, moreover let $\mathcal{L}\subseteq\Sigma^*$ and $a\in\Sigma$, then:
\[
 \mathcal L\bigl(\mathcal D_a^\cut( r)\bigr)
  \subseteq
 \{w\in \Sigma^\star \mid q^\star_\mu(w)\in \mathcal L\bigl(\mathcal D_{[a]_\cut} [r]_\mu\bigr)\}= \]
\[=\{w\in \Sigma^\star \mid q^\star_\mu(w)\in \partial_{[a]_\mu}(\mathcal L([r]_\mu)\}= \partial_a^\mu(\mathcal L(r))\]
...
If no $x\in S$ is partially included, \ie $\mu_S(x)\in\{0,1\}$ for all $x$, the fuzzy set is said to be {\em crisp}.
\end{definition}
...
\begin{enumerate}
    \item G\"odel minimum T-norm: $a\otimes b =\min(a,b)$, whose residuum is \\$a\Rightarrow b = \begin{cases}b & \mbox{ if } b\lneq a \\
...
\begin{definition}[Fuzzy and T-similarity relations]\label{def:fuzzy-relation}
\hfill
\begin{enumerate}
...
A fuzzy relation $\cR$ on a set $S$ is
called a \emph{T-similarity relation} (on $S$) iff it is reflexive, symmetric, and satisfies the reverse
...
\end{description}
\item A T-similarity relation (on $S$) is
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\noindent {\color{red}\bf ALL FOLLOWING RESULTS UP TO END OF SECTION SHOULD BE CONSEQUENCES OF THE MAIN THEOREM: do we need to spell them out for the special case? NOT CHECKED}
