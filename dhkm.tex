...
The special case of the present work for the minimum T-norm appeared as an extended abstract, by a subset of the authors, at Women in Logic (WiL 2026)~\cite{KM26}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
recalled in Appendix~\ref{appendix}.  {\color{red} well if you recall it then there is no point in putting it in the appendix}. In particular, recall from
...
\begin{lemma}\label{T-equivalence} The $\mu$-cut of a T-similarity relation is an equivalence if and only if $\mu$ id idempotent for $T$
\end{lemma}
Hence if all $\mu$-cuts of a T-norm are equivalences the norm is globally idempotent and hence necessarily G\"odel's minimum T-norm. Nevertheless, there are many non-idempotent T-norms which are locally idempotent, \eg\ 
...
Viewing words as regular expressions,  Definition~\ref{def:Rw} naturally extends to regular expressions, when the T-norm is globally idempotent on $\mu$ \ie\ if we take G\"odel's T-norm.
\begin{definition}[T-similarity on Regular Expressions (locally idempotent case)] \label{def:Rexp}Given a similarity relation $\mathcal R$ on the alphabet, define a proximity on regular expressions,  $\mathcal{G}^{E}:E \times E \rightarrow [0,1]$, as follows
...
If we consider arbitrary T-norms whose values are not globally idempotent, then, Definition~\ref{def:Rexp} is both too generous over regular expressions in the case of  the $(\ )^\star$-operator and too strict in the case of the $\ +\ $constructor and the correspondence with language similarity as in Theorem~\ref{thm:Rr-RL-bridge} fails.  
We have the following counterexamples. Assume $\mathcal R(a,b)\lneq 1$ then $\mathcal R^L(\mathcal L(a^*),\mathcal L(b^*))= \mathcal R(a,b)$ only if  the T-norm is locally idempotent on $\mathcal R(a,b)$. Assume
\(\mathcal R(a,c)=\mathcal R(b,d)=0.5\), then \(\mathcal R^L(\mathcal L(r_1),\mathcal L(r_2))=0.5\) while \(\mathcal R^E(a+b,c+d)=0.5\cdot0.5=0.25\).
We need to use the $\min$ operator in the $+$ clause in Definition~\ref{def:Rexpg}. 
Thus, Definition~\ref{def:Rexp} is conservatively extended as follows.
...
{\color{red} This notion has to be discussed here}
...
\item We can incorporate in the above definition finite equational properties among regular expressions, see \eg\ \ref{lem:similarity-sound} and we can put $\mathcal R^E(r_1,s) = \mathcal R^E(r_2,s)$ if ${\sf nf}(r_1)={\sf nf}(r_2)$. But since no exhaustive finitary definition can be given, we shall not pursue this.
\item Theorem~\ref{thm:Rr-RL-bridge} is not true for regular expressions when $\&$ and $\neg$ are
...
\subsection{Fuzzy Derivatives - T-norm locally idempotent on $\mu$}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Throughout this section we assume that  the T-norm is locally dempotent on $\mu$, as \eg\ in  G\"odel's minimum T-norm.
...
Capitalizing on the the idempotency of $\mu$, fuzzy derivatives can be rephrased as follows,  see \cite{KM26} .
...
Recalling that the T-norm is idempotent on $\mu$, as in the case of G\"odel's T-norm,  Definition~\ref{def:regex-fuzzy-derivative} of $\mu$-fuzzy expression derivatives simplifies in the following way.
