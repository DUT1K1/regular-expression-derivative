...
\item {\color{red}a fuzzy relation is {\em crisp} if the value of $\cR$ is always 1;}
...
\item {\color{red} a fuzzy relation $\cR$ is a {\em proximity relation} if it is reflexive and symmetric};
...
\item {\color{red} A T-similarity relation $\cR$ is said to be {\em g\"odelian} if the T-norm, $\otimes$, is idempotent on all the values of the pairs in $\cR$.
}
\end{enumerate}
\end{definition} 

{\color{red} Notice that a proximity relation can be a T-similarity relation w.r.t. different norms. An important role in the sequel will be played  by proximity relations which are equivalences.}
The following Lemma is immediate.
\begin{lemma}\label{T-equivalence} The $\mu$-cut of a T-similarity relation, $\cR$ is an equivalence relation if $\otimes$ is locally idempotent on $\mu$, \ie $\mu\otimes\mu=\mu$.
\end{lemma}
{\color{red} Rocq proof??}\vspace{0.5cm}

{\color{red} Clearly $\mu$-cuts of g\"odelian T-similarity relations are equivalences. However, all crisp equivalence relations are T-similarity relations for any norm, and hence Lemma~\ref{T-equivalence} cannot be reversed.}


If  a norm is globally idempotent, and hence necessarily G\"odel's {\em minimum T-norm}, see Definition~\ref{norms}.1, is obvioiusly g\"odelian.  Nevertheless, there are many non-idempotent T-norms which are locally idempotent on intervals in $[0,1]$, \eg\ the ordinal sum of the product and the minimum T-norm:
$$T(x,y)=\begin{cases}p+ \dfrac{(x-p)(y-p)}{1-p} & x,y\geq p;\\ \min(x,y) & \mbox{ otherwise,}
\end{cases}$$ where $p\in [0,1]$. Such T-norms therefore induce $\mu$-cuts which are equivalences only for certain values of $\mu$.
...
{\color{red} Given a T-similarity relation $\mathcal R^w$ on $\Sigma^*$,} for languages $L_1,L_2\subseteq \Sigma^*$ define a fuzzy relation $\mathcal{R}^L:\mathcal P(\Sigma^*)\times \mathcal P(\Sigma^*)\to [0,1]$ as follows
\[
\mathcal{R}^L(L_1,L_2)\;=\;
\min\!\Bigl\{
...
\(\mathcal R(a,c)=\mathcal R(b,d)=0.5\), {\color{red} and that the norm is Goguen's product norm~\ref{norms}.2} then \(\mathcal R^L(\mathcal L(r_1),\mathcal L(r_2))=0.5\) while \(\mathcal R^E(a+b,c+d)=0.5\cdot0.5=0.25\).\\
...
\begin{lemma} \label{T-similarity}{\color{red}Given a T-similarity relation $\mathcal R$ on $\Sigma$} \begin{enumerate}
...
\begin{theorem}\label{thm:Rr-RL-bridge}Given a g\"odelian T-simlarity relation, $\cR$, on the alphabet $\Sigma$, for all restricted regular expressions $r_1$ and $r_2$,
\begin{enumerate} 
...
$0<\mu\le1$, $ u\in \mathcal L^\mu(r)$ if and only if $\epsilon \in \mathcal L(
\mathcal D_u^\mu(r)).
$
...
\subsection{The special case $\mu\otimes\mu=\mu$}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Throughout this section we rephrase their results for the case of a general T-norm which is locally idempotent on $\mu$, \ie $\mu\otimes\mu=\mu$. Capitalizing on this local idempotency all definitions and theorems of Section~\ref{sec:fuzzyderivatives} greatly simplify due to Lemma~\ref{T-equivalence}, and all $\mu$-neighborhoods of symbols, $a$ over $\Sigma$, \ie $\{b \in\Sigma :  (a,b)\in\mathcal{R}_\cut\}$, become equivalence classes. 
...
\noindent {\color{red}\bf  ALL  FOLLOWING RESULTS UP TO END OF SECTION  ARE COSEQUENCES OF THE MAIN THEOREM: do we need to spell them out for the special case?}
