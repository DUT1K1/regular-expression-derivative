...
\item a fuzzy relation is {\em crisp} if the value of $\cR$ is always 1;
...
{\color{red}\begin{conjecture} Let $r,s$ be restricted regular expressions over $\Sigma$, and let $\mathcal{R}$ be a T-similarity relation on $\Sigma$, then for all restricted regular expressions $r_1,r_2$  $\mathcal{R}^E(r,s)\gneq 0$ if and only if $r_1=r[a_1,\ldots,b_n] $ and $r_2=r[b_1,\ldots,b_n] $ for suitable choices of $r$ and $a_i$'s and $b_i$'s. 
\end{conjecture}}
...
\partial_a^\mu(L(r)).
$
\item{\color{red} (Strong Semantic-syntactic Correspondence)  For every regular expression $r$, symbol \mbox{$a\in\Sigma$}, and cut value
$0<\mu\le1$, $
L^\mu(\mathcal D_a(r)) =L^\mu(\mathcal D^\mu_a(r))
=
\partial_a^\mu(L(r)).
$}
...
Let $\cR$ be a T-similarity relation on $\Sigma$ and let $\cut$ be a cut value
with $0 < \cut \le 1$ such that $\mu\otimes\mu=\mu$ then 
...
{\color{red}Notice that if we have also the following: 
\begin{proposition}\label{prop:cut-equivalence}
...
Let $\cR$ be a T-similarity relation on $\Sigma$ and let $\cut$ be a cut value
with $0 < \cut \le 1$ such that such that $\mu\otimes\mu = \mu$, moreover let  $\mathcal{L}\subseteq\Sigma^*$ and $a\in\Sigma$. 
\begin{enumerate}
...
Proposition~\ref{the:quotient-fuzzy-derivative} below provides the explanation for  all the above simplifications, by showing that in the case $\cR^\mu$ is an equivalence then the fuzzy derivatives becomean ordinary derivatives if we pass to the quotient alphabet, {\em i.e.}, once letters are identified up to
...
Let $\cR$ be a T-similarity relation on $\Sigma$ and let $\cut$ be a cut value
with $0 < \cut \le 1$ such that such that $\mu\otimes\mu = \mu$, moreover let  $\mathcal{L}\subseteq\Sigma^*$ and $a\in\Sigma$, then:
\[
  \mathcal L\bigl(\mathcal D_a^\cut( r)\bigr)
  \subseteq
  \mathcal D_{[a]_\cut}\bigl( [r]_\mu).
\]
\end{remark}}\vspace{1cm}
