...
A preliminary version of this work, by a subset of the authors, appeared as an extended abstract at Women in Logic (WiL 2026)~\cite{KM26}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
For languages $L_1,L_2\subseteq \Sigma^*$ define a proxmity relation $\mathcal{R}^L:\mathcal P(\Sigma^\ast)\times \mathcal P(\Sigma^\ast)\to [0,1]$ as follows
\[
\mathcal{R}^L(L_1,L_2)\;=\;
\min\!\Bigl\{
...
We need to use the $\min$ operator in the $+$ clause in Definition~\ref{def:Rexpg} because otherwise Theorem~\ref{thm:Rr-RL-bridge} fails because of the following coounterexample: 
\(\mathcal R(a,c)=\mathcal R(b,d)=0.5\), and take \(r_1=a+b\) and \(r_2=c+d\). 
Then \(\mathcal R^L(L(r_1),L(r_2))=0.5\) while \(\mathcal R^E(r_1,r_2)=0.5\cdot0.5=0.25\)
...
\begin{theorem}\label{thm:Rr-RL-bridge}For all restricted regular expressions $r_1$ and $r_2$,
\begin{enumerate} 
\item$\mathcal R^E(r_1,r_2)\leq \mathcal R^L(L(r_1),L(r_2));$

%\item{\color{blue}$\mathcal{R}^L\bigl(L(r_1),L(r_2)\bigr)=\sup\limits_{\substack{L(r_1)=L(r'_1)\\L(r_2)=L(r'_2)}}\mathcal{R}^E(r_1',r_2')$}
...
\item Given regular expressions $r_1,r_2\in R^\Sigma$ and words $w_1\in L(r_1)$ and $w_2\in L(r_2)$ of the same length, we may have  that $\mathcal{R}^{E}(r_1,r_2)\; \ngeq \; \mathcal R^w(w_1,w_2)$, as well as $\mathcal{R}^{E}(r_1,r_2)\; \nleq \; \mathcal R^w(w_1,w_2)$, \eg $R^w(a,a)=1$ but $\mathcal{R}^{E}(a,a+a)=0$, and $R^w(a,b)=0$ but $\mathcal{R}^{E}(a+b,a+b)=1.$
...
\item %Lemma~\ref{thm:syntax-semantics}  and 
Theorem~\ref{thm:Rr-RL-bridge} is not true for  regular expressions when $\wedge$ and $\neg$ are allowed. For instance, if $\mathcal R(a,b)\gneq 0$ but $a\neq b$ we have that $L(a\cap b)=\emptyset$ and hence $\mathcal R^L(L(a\cap a),L(a\cap b))=0$ but $\mathcal R^E((a\cap a),(a\cap b))=\mathcal R(a,b)$. We have also that $\mathcal R^L(L(\neg a),L(\neg b))$ is unrelated to $R(a,b)\gneq 0$ , but depends on the existence of other characters in the alphabet. And hence it does not seem exèresisble. {\color{red} Do you have a better argument?} 
%The reason is that $\mathcal{R}^E$ only compares the shape of the expressions. So $\mathcal{R}^E$ can be positive as long as the corresponding subexpressions look similar, even if the crisp languages are empty or different.
\end{enumerate}
\end{remark}
...
\begin{definition}\label{def:regex-fuzzy-derivative-godel}
Let $r$ be a restricted regular expression over $\Sigma$, let $a\in\Sigma$, and let
$0<\cut\le 1$. The \emph{fuzzy derivative} of $r$ with respect to $a$ and
$\cut$ is
\[
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Since the introduction of fuzzy automata by Santos~\cite{Santos68} and Wee~\cite{Wee67,WeeFu69} in the late 1960s, many authors have addressed the topic of fuzzyfying the theory of regular languages. Most of these approaches have taken the standard view of fuzzy sets as functions in some lattice ordered monoid. In an important line of research, see \cite{LP06,SC12,GJT17}, fuzzification of regular expressions is achieved by introducing a scalar $\lambda\in \mathcal L$ to the algebra of regular expressions with the intendend meaning that membership to the induced language is multiplied by $\lambda$. Our approach is somewhat different, since we have taken a more quantitative and metric view, using the notion of proximity and hence similarity in the forefront. 
...
\begin{definition}A fuzzy set is a pair $(S, \mu_S)$, where $S$ is a set and $\mu_S: S\rightarrow [0,1]$ is a function. %The set $\univ$ is called the universe of discourse, and
For all $x\in S,\, \mu_S(x)$ is called the degree of membership of $x$. We say that:
\begin{itemize}
    \item  $x$ is \emph{fully included} in $(S, \mu_S)$, if $\mu_S = 1$;
    \item  $x$ is \emph{not included} in $(S, \mu_S)$, if $\mu_S(x) = 0$;
    \item  $x$ is \emph{partially included} in $(S, \mu_S)$, if $0<\mu_S(x) < 1$.
\end{itemize}
If  all $x\in S$ are fully included in $S$, the fuzzy set $S$ is said to be {\em crisp}.
\end{definition}
...
A \emph{T-norm}, see \cite{met05}, is a binary function $T: [0, 1] \times [0, 1] \to [0, 1]$ that satisfies the following properties:
\begin{itemize}
    \item commutativity: $T(a, b) = T(b, a)$,
    \item associativity: $T(a, T(b, c)) = T(T(a, b), c)$,
    \item monotonicity: $T(a, b) \leq T(c, d)$ if $a \leq c$ and $b \leq d $,
    \item the number 1 is the identity element: $T(a, 1) = a$.
\end{itemize}
\end{definition}

\noindent Usually, T-norms are written in infix notation and the symbol $\otimes$ is used to denote them. In the paper, we consider only left-continuous t-norms. 


\begin{definition}[Residuum] \label{residuum}A binary function $\otimes: [0,1]^2 \rightarrow [0, 1] $ is said to be residuate         
...
\begin{itemize}
    \item G\"odel T-norm: $a\otimes b =\min(a,b)$, whose residuum is \\$a\Rightarrow b = \begin{cases}b & \mbox{ if } b\lneq a \\
...
\item\label{first2} $\Rightarrow$ is monotone in the second argument and anti-monotone in its first argument;
...
\begin{proof}
\begin{itemize}
\item Property (1) follows by the adjunction in Definition~\ref{residuum}.
\item Properties (2)-(5) follow from previous items and the Definition~\ref{residuum}. We illustrate (5). From the adjunction we have that  $1\leq a\Rightarrow b \Longleftrightarrow 1\otimes a \leq b$, but $a=1\otimes a$ by (2).
\item Property (6) follows from monotonicity of the T-norm and identity.
\item Property (7) follows from the adjunction of the T-norm.
\item Property (8) follows from the fact that, by adjunction we have $a\otimes (a\Rightarrow b) \leq b \Leftrightarrow  (a\Rightarrow b) \leq (a\Rightarrow b)$, and  hence  $a\otimes (a\Rightarrow b) \leq b$ always holds.
\end{itemize}
\end{proof}

\begin{definition}[Similarity Relation]\label{similarity}
A {\em similarity relation}, $S$, over a domain $D$, is a fuzzy subset of $D\times D$, which is reflexive, symmetric, and transitive.\\ Namely let $x,y,z\in D$ and let $\mu_S(x,y)$ denote the grade of membership of the ordered pair $(x,y)\in D\times D$. Then $S$ is a similarity relation in D if and only if, for all $x, y, z \in D$, \begin{itemize}
    \item 
$\mu_S(x,x)= 1$(reflexivity), 
\item $\mu_S(x,y)= 1$ implies $\mu_S(y,x)= 1$ (symmetry),
\item $\mu_S(x,z) \geq \mu_S(x,y) \otimes \mu_S(y,z)$ (transitivity).
\end{itemize}
\end{definition}
