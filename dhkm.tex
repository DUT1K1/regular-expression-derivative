...
For languages $L_1,L_2\subseteq \Sigma^*$ define a proximity relation $\mathcal{R}^L:\mathcal P(\Sigma^*)\times \mathcal P(\Sigma^*)\to [0,1]$ as follows
\[
\mathcal{R}^L(L_1,L_2)\;=\;
\min\!\Bigl\{
...
\begin{definition}[Similarity on Regular Expressions - G\"odel's T-norm] \label{def:Rexp}Given a similarity relation $\mathcal R$ on the alphabet, define a proximity on regular expressions,  $\mathcal{G}^{E}:E \times E \rightarrow [0,1]$, as follows
...
Let $r,s$ be restricted regular expressions over $\Sigma$, and let $\mathcal{R}$ be a similarity on $\Sigma$. Define inductively the following proximity relation
...
We need to use the $\min$ operator in the $+$ clause in Definition~\ref{def:Rexpg} since otherwise Theorem~\ref{thm:Rr-RL-bridge} fails because of the following counterexample:
...
\begin{definition}\label{def:derivative}
    The derivative of a language $L \subseteq \Sigma^*$ with respect to a symbol $a \in \Sigma$ is defined to be:
    $$
...
language derivative} of $\mathcal{L}$ with respect to $a$ is
$$
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Throughout this section we take as T-norm G\"odel's minimum T-norm.
...
after passing to the quotient alphabet, {\em i.e.}, once letters are identified up to
...
Taking G\"odel's T-norm,  Definition~\ref{def:regex-fuzzy-derivative} of $\mu$-fuzzy expression derivatives simplifies in the following way.
...
\end{proof}
 ...
 If we were to take derivatives up to semantic equivalence, then we would get minimal automata. If we take derivatives only up to canonical form, then this is not the case. For example, consider the restricted regular expressions $a^*$ and $(\varepsilon+a)^*$. These are semantically equivalent but are not canonically equal.
...
\begin{itemize}
    \item  $x$ is \emph{fully included} in $(S, \mu_S)$, if $\mu_S(x) = 1$;
...
\item Properties (2)-(5) follow from previous items and Definition~\ref{residuum}. We illustrate (5). From the adjunction we have that  $1\leq a\Rightarrow b \Longleftrightarrow 1\otimes a \leq b$, but $a=1\otimes a$ by (2).
