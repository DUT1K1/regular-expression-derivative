...
For languages $L_1,L_2\subseteq \Sigma^*$ define a proxmity relation $\mathcal{R}^L:\mathcal P(\Sigma^*)\times \mathcal P(\Sigma^*)\to [0,1]$ as follows
\[
\mathcal{R}^L(L_1,L_2)\;=\;
\min\!\Bigl\{
...
If we consider arbitrary T-norms, Definition~\ref{def:Rexp} is too generous over regular expressions. If $\mathcal R(a,b)\lneq 1$ then $\mathcal R^L(\mathcal L(a^*),\mathcal L(b^*))= \mathcal R(a,b)$ only if  $\mathcal R(a,b)$ is idempotent. But a norm where all values are idempotent is necessarily G\"odel's minimum T-norm. 
...
Then \(\mathcal R^L(\mathcal L(r_1),\mathcal L(r_2))=0.5\) while \(\mathcal R^E(r_1,r_2)=0.5\cdot0.5=0.25\)

The definition of similarity for $(\ )^*$ is indeed rather strict for a general T-norm, but no definition for a general T-norm seems reasonable unless we replace it with a finite union. 
...
%\[\emptyset,\ \varepsilon,\ a \in \Sigma\]using only the constructors $+$, $\cdot$, and ${}^*$.Equivalently, restricted regular expressions are those that do not use $\wedge$ or $\neg$.\end{definition}
...
{\color{blue}
Theorem~\ref{thm:Rr-RL-bridge} is not true for regular expressions when $\&$ and $\neg$ are
allowed. For instance, if $\mathcal R(a,b)\gneq 0$ but $a\neq b$, we have that
$\mathcal L(a\,\&\,b)=\emptyset$ and hence
$\mathcal R^L(\mathcal L(a\,\&\,a),\mathcal L(a\,\&\,b))=0$, but
$\mathcal R^E(a\,\&\,a,\,a\,\&\,b)=\mathcal R(a,b)$. Similarly, over $\Sigma=\{a,b\}$ we have
...
\item The relation on $\mathcal P(\Sigma^*)$
...
\end{proof}
 ...
 If we would take derivatives up to semantic equivalence then we would get minimal automata. If we take derivatives only up to canonical form than this is not the case. For example consider the restricted regualr expressions $a^*$ and $(\varepsilon+a)^*$. These are semantically equivalent but are not canonically equal.
The Brzozowski automaton that we get has two derivatives, namely itself and $\mathcal D_a((\varepsilon+a)^*) =\mathcal D_a(\varepsilon+a)\cdot(\varepsilon+a)^*=\ldots =a^*$, but $a^*$ is its own derivative. 
