...
\paragraph{Notation} Regular expressions, possibly restricted, are denoted by $r,s$, possibly with indices or superscripts. We write $r[a_1,\ldots,a_n]$ to highlight the occurrences of the alphabet symbols $a_1,\ldots,a_n$ in $r$. By $r[s_1,\ldots,s_n]$ we denote the regular expression obtained by replacing $a_1,\ldots,a_n$ with $s_1,\ldots,s_n$ in $r[a_1,\ldots,a_n]$. We write $r\equiv s$, and say that $r$ and $s$ are \emph{semantically equivalent}, when $\mathcal L(r)=\mathcal L(s)$.
...
Throughout, $\otimes$ denotes a left-continuous T-norm on $[0,1]$, written in infix
notation. We use the standard notions of fuzzy logic --- fuzzy sets, T-norms and their
residua, fuzzy relations, and proximity and T-similarity relations --- following
\cite{met05,Sessa2002}; these, together with the three standard examples of T-norms, are
recalled in Appendix~\ref{appendix}. In particular, recall from
Definitions~\ref{T-norm} and~\ref{def:fuzzy-relation} that a \emph{fuzzy relation} on a
set $S$ is a map $\cR:S\times S\to[0,1]$; that its \emph{$\cut$-cut} at a \emph{cut value}
$0<\cut\le 1$ is the crisp relation
\[
  \cR_\cut := \{(s_1,s_2) \mid \cR(s_1,s_2) \ge \cut \};
\]
that $\cR$ is a \emph{proximity relation} when it is reflexive and symmetric; and that a
proximity relation is a \emph{T-similarity relation} when it also satisfies the reverse
triangle inequality $\cR(s_1,s_2)\ge \cR(s_1,s) \otimes \cR(s,s_2)$. When the T-norm is
G\"odel's \emph{minimum} T-norm, we often write $\min$ instead of $\otimes$.
...
coincide under the semantics map.

\begin{theorem}[Classical properties of derivatives~\cite{B64}]
...
\begin{enumerate}
  \item $\partial_u(L)$ is regular;
  \item (semantic--syntactic correspondence)
        $\mathcal{L}\bigl(\mathcal{D}_{u}(r)\bigr)=\partial_{u}\bigl(\mathcal{L}(r)\bigr)$;
  \item the set of derivatives $\bigl\{\, \partial_{u}(L) \mid u \in \Sigma^* \bigr\}$ is
        finite.
\end{enumerate}
\end{theorem}
