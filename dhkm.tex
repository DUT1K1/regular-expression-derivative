...
Throughout the paper, $\otimes$ denotes a left-continuous T-norm on $[0,1]$, written in infix
...
\begin{lemma}\label{T-equivalence} The $\mu$-cut of a T-similarity relation is an equivalence if and only if $T$ is locally idempotent on $\mu$.
\end{lemma}
Hence if all $\mu$-cuts of a T-norm are equivalences the norm is globally idempotent and hence necessarily G\"odel's minimum T-norm. Nevertheless, there are many non-idempotent T-norms which are locally idempotent on intervals in $[0,1]$, \eg\ the ordinal sum of the product and the minimum T-norm:
...
Given a T-similarity between symbols $\mathcal{R}:\Sigma \times \Sigma \rightarrow [0,1]$ we  define T-similarities between words, regular expressios, and languages as follows, see Lemma~\ref{T-similarity}.
...
Viewing words as regular expressions,  Definition~\ref{def:Rw} naturally extends to regular expressions, when the T-norm is globally idempotent \ie if we take G\"odel's T-norm.
\begin{definition}[T-similarity on Regular Expressions (globally idempotent case)] \label{def:Rexp}Given a similarity relation $\mathcal R$ on the alphabet, define a proximity on regular expressions,  $\mathcal{G}^{E}:E \times E \rightarrow [0,1]$, as follows
