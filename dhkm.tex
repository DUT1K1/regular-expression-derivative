...
In this paper we use standard notions in fuzzy logic, see~Appendix~\ref{appendix} or \cite{met05,Sessa2002} for more details. Throughout the paper, $\otimes$ denotes a left-continuous T-norm, see Definition~\ref{T-norm}, on the real interval $[0,1]$, written in infix
notation. 
%fuzzy sets, T-norms and their
%residua, fuzzy relations, and proximity and T-similarity relations --- following
%hree standard examples of T-norms, are
...
\item a fuzzy relation, $\cR$, is a \emph{T-similarity relation} if it is reflexive and symmetric and it satisfies the reverse
triangle inequality with respect to the T-norm, $\otimes$, $\cR(s_1,s_2)\ge \cR(s_1,s) \otimes \cR(s,s_2)$.
\end{enumerate}
\end{definition} 
%
%When the T-norm is
%G\"odel's \emph{minimum} T-norm,.
The following Lemma is immediate.
\begin{lemma}\label{T-equivalence} The $\mu$-cut of a T-similarity relation, $\cR$ is an equivalence relation if and only if $\otimes$ is locally idempotent on $\mu$, \ie $\mu\otimes\mu$.
\end{lemma}
{\color{red} Rocq proof??}\vspace{0.5cm}

Hence, if all $\mu$-cuts of a T-norm are equivalences the norm is globally idempotent and hence necessarily G\"odel's {\em minimum T-norm}, see Definition~\cite{norm}.1  which we often write $\min$. Nevertheless, there are many non-idempotent T-norms which are locally idempotent on intervals in $[0,1]$, \eg\ the ordinal sum of the product and the minimum T-norm:
$$T(x,y)=\begin{cases}p+ \dfrac{(x-p)(y-p)}{1-p} & x,y\geq p;\\ \min(x,y) & \mbox{ otherwise,}
\end{cases}$$ where $p\in [0,1]$. Such T-norms induce $\mu$-cuts which are equivalences only for certain values of $\mu$.


\subsection{T-similarities on Words, Regular Expresions, and Languages}
Given a T-similarity between symbols $\mathcal{R}:\Sigma \times \Sigma \rightarrow [0,1]$ we  define fuzzy relations between words, regular expressios, and languages as follows. All such fuzzy relations are in fact T-similarities, see Lemma~\ref{T-similarity}.
...
For languages $L_1,L_2\subseteq \Sigma^*$ define a fuzzy relation $\mathcal{R}^L:\mathcal P(\Sigma^*)\times \mathcal P(\Sigma^*)\to [0,1]$ as follows
\[
\mathcal{R}^L(L_1,L_2)\;=\;
\min\!\Bigl\{
...
\begin{definition}[T-similarity on Regular Expressions for G\"odel's T-norm] \label{def:Rexp}Given a similarity relation $\mathcal R$ on the alphabet, define a proximity on regular expressions,  $\mathcal{G}^{E}:E \times E \rightarrow [0,1]$, as follows
...
We have the following counterexamples.\\ 1) Assume $\mathcal R(a,b)\lneq 1$ then $\mathcal R^L(\mathcal L(a^*),\mathcal L(b^*))= \mathcal R(a,b)$ only if  the T-norm is locally idempotent on $\mathcal R(a,b)$. \\ 2) Assume
...
\begin{lemma} \label{T-similarity}\hfill \begin{enumerate}
...
\begin{enumerate} 
\item if $\mathcal R^E(r_1,r_2)\geq \mu$ then $L^\mu(r_1)=L^\mu(r_2)$;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In this section we recall standard definitions and properties of Brzozowski derivatives, see~\cite{} for more details. First we need to introduce the notion of nullable regular expression. 
...
\begin{theorem}[Classical properties of derivatives~\cite{B64}]\label{the:derivative}
...
\begin{enumerate}
\item $u\in \mathcal L(r)$ if and only if $\epsilon \in \mathcal{D}_{u}(r)$.
...
\item (syntactic finiteness) the set of derivatives $\bigl\{\, \mathcal D_{u}(L) \mid u \in \mathcal L(r) \bigr\}$ is
        finite up to canonicalization.
  \item (semantic finiteness) the set of derivatives $\bigl\{\, \partial_{u}(L) \mid u \in L \bigr\}$ is
        finite.
\end{enumerate}
\end{theorem}
...
\item If $\Sigma$ is finite for every regular expression $r$, word $u\in \Sigma^\ast$, and cut value
$0<\mu\le1$, $ u\in \mathcal L^\mu(r)$ if and only if $\epsilon \in
\mathcal D_u^\mu(r).
$
...
\item (Semantic-syntactic Correspondence) For every regular expression $r$, symbol \mbox{$a\in\Sigma$}, and cut value
$0<\mu\le1$, $
L(\mathcal D_a^\mu(r))
=
\partial_a^\mu(L(r)).
...
$
\{\mathcal D_u^\mu(L)\mid u\in\mathcal L(r)\}$ is finite up to canonicalization.
...
$
\{\partial_u^\mu(L)\mid u\in L\}
$
is finite. 
\end{enumerate}
\end{theorem}

{\color{red}\bf THIS IS THE MAIN THEOREM AND THE FIRST ITEM IS THE REASON WHY WE DO IT}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{The special case $\mu\otimes\mu$}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
This section is a minor generalization of the results in~\cite{KM26} which were origianlly given for G\"odel's minimum T-norm, see Definition~\ref{norms}. 
Throughout this section we rephrase their results for the case of a general T-norm which is locally dempotent on $\mu$, \ie $\mu\otimes\mu=\mu$. Capitalizing on this local idempotency all definitions and theorems of Section~\ref{sec:fuzzyderivatives} greatly simplify due to Lemma~\ref{T-equivalence}, and all $\mu$-neighborhoods of symbols, $a$ over $\Sigma$, \ie $\{b \in\Sigma :  (a,b)\in\mathcal{R}_\cut\}$, become equivalence classes. 

\begin{proposition}\label{prop:cut-equivalence}
Let $\cR$ be a fuzzy similarity relation on $\Sigma$ and let $\cut$ be a cut value
with $0 < \cut \le 1$ such that $\mu\otimes\mu=\mu$.
The following hold. 
\begin{enumerate}
\item  The relation $
...
Let $\mu\otimes\mu=\mu$, $\mathcal{L}\subseteq\Sigma^*$ and $a\in\Sigma$. 
\begin{enumerate}
\item \label{def:regex-fuzzy-derivative-godel}
Let $r$ be a restricted regular expression over $\Sigma$, let $a\in\Sigma$.The $\mu$-fuzzy syntactic derivative of $r$ with respect to $a$ anfd
$\cut$ is equal to
\[
  \mathcal D_a^\cut(r)
  =
...
Proposition~\ref{the:quotient-fuzzy-derivative} explains all the above simplifications, by showing that in the case of $\mu\otimes\mu=\mu$ the fuzzy derivatives becomean ordinary derivatives if we pass to the quotient alphabet, {\em i.e.}, once letters are identified up to
...
Let $\mu\otimes\mu=\mu$, $\Sigma^\cut := \Sigma/{\sim_\cut^\Sigma}$, and let
...
Let $\cR$ be a fuzzy similarity relation on $\Sigma$ and let $\cut$ be a cut value
with $0 < \cut \le 1$ such that $\mu\otimes\mu=\mu$. For every language $L \subseteq \Sigma^*$ and every
$a\in\Sigma$,
\[
  \partial_a^\cut( L)
  \;=\;
  \{w\in\Sigma^* \mid q_\cut^*(w)\in
       \partial_{[a]_\cut}\bigl(L^\cut\bigr)\}.
\]
Hence,
\[
...
{\color{red} OR SHCOULD WE PHRASE IT AS IN THE WIL ABSTRACT? \\\begin{theorem}\label{main} If $a\in\Sigma$ and $L\subseteq\Sigma^\ast$ is regular, then for every cut value $0<\mu\le1$, we have \mbox{$
\partial_a^\mu(L)
=
\{w\in\Sigma^\ast\mid
q_\mu^\ast(w)\in
...
\begin{proof}[Proof sketch]{\color{red} not checked}
...
{\color{red}  FOLLOWING RESULTS UP TO END OF SECTION  ARE COSEQUENCE OF THE MAIN THEOREM: do we need it?}
