...
\begin{proof}[Proof.]
Since $L$ is regular, there exists a deterministic finite automaton
$M=(Q,\Sigma,\delta,q_0,F)$ with $\mathcal{L}(M)=L$ and $Q$ finite.
We extend $\delta$ to words by
\[
\delta(q,\varepsilon)=q
\quad\text{and}\quad
\delta(q,xa)=\delta(\delta(q,x),a)\quad\text{for }x\in\Sigma^*,\ a\in\Sigma.
\]

For each $w\in\Sigma^*$, define $q_w \mathrel{:=} \delta(q_0,w)$.
Then
\[
u \in \partial_w(L)
\iff w\cdot u \in L
\iff \delta(q_0,w\cdot u)\in F
\iff \delta(\delta(q_0,w),u)\in F
\iff \delta(q_w,u)\in F.
\]
Hence
\[
\partial_w(L)
= \{\,u\in\Sigma^* \mid w\cdot u \in L \,\}
= \{\,u\in\Sigma^* \mid \delta(q_w,u)\in F\,\}.
\]



Consequently, if $q_w=q_v$ then $\partial_w(L)=\partial_v(L)$.
After reading $w$ and after reading $v$ the automaton is in the same state.
From that same state, every continuation $u$ is processed identically, so $wu$ is accepted iff $vu$ is accepted.
Therefore the sets $\{\,u \in \Sigma^* \mid wu \in L\,\}$ and $\{\,u \in \Sigma^* \mid vu \in L\,\}$ coincide.


Because the automaton has only finitely many states, there are only finitely many possibilities for the state reached after reading a prefix $w$.
Whenever two prefixes lead to the same state, they yield the same derivative.
Hence distinct derivatives can arise only when the resulting states after those prefixes are different.
There are at most $|Q|$ such states, so there are at most $|Q|$ distinct derivatives $\partial_w(L)$; Therefore, the set $\{\partial_w(L)\mid w\in\Sigma^*\}$ is finite.
