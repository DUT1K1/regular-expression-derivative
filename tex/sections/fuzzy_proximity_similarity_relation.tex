...
\min\{\mathcal{R}^r(r_1,s_1),\mathcal{R}^r(r_2,s_2)\}
...
Therefore two regular expressions may denote the same language while having low, or even zero, $\mathcal{R}^r$-similarity.
For example, depending on the base relation on symbols, one may have
\[
\mathcal{R}^r(a+b,b+a)=0
\]
...
\begin{theorem}\label{thm:Re-RL-bridge}
For all regular expressions $r$ and $s$,
\[
\mathcal{R}^L\bigl(L(r),L(s)\bigr)
=
\sup\Bigl\{
\min\bigl(\mathcal{R}^r(r',t),\mathcal{R}^r(s',u)\bigr)
\;\Bigm|\;
L(r')=L(r),\;
L(s')=L(s),\;
L(t)=L(u)
