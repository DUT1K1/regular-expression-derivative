...
{\color{red} If we take derivatives up to semantic equivalence then we get minimal automata. If we take derivatives up to canonical from than this is not the case. For example consider $a^\ast$ and $(\epsilon+a)^\ast$. The Brzozowski automaton that we get has two derivatives itself and $\mathcal D_a((\epsilon+a)^\ast) =\mathcal D_a(\epsilon+a)\cdot(\epsilon+a)^\ast=\ldots =a^\ast$, but $a^\ast$ has only one derivative. The derivatives are not canonically equal.}
