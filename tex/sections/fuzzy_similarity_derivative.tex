...
{\color{blue}
\begin{proposition}
Let $\cR$ is a fuzzy similarity relation on $\Sigma$ and let $\cut$ be a cut value ($0 < \cut \le 1$). The relation $a\sim_\mu^\Sigma b$ if and only if $\cR(a,b)\geq \mu$ is an equivalence relation on $\Sigma$, which induces naturally an equivalence relation on words, $w_1\sim_\mu^{\Sigma^\star} w_2$.
\end{proposition}

\begin{definition}Let $\cR$ be a fuzzy similarity relation on $\Sigma$ and let $\cut$ be a cut value ($0 < \cut \le 1$). Let \begin{itemize}
\item $\Sigma^\mu$ be the quotient set of $\Sigma$ w.r.t. the equivalence relation $\sim_\mu^\Sigma$;
\item $\mathcal L^\mu$ be the quotient set of $\mathcal L$ with respect to the equivalence  $\sim_\mu^{\Sigma^\star}$, which is a language on $\Sigma^\mu$.
\end{itemize}
\end{definition}

\begin{theorem}Let $\cR$ be a fuzzy similarity relation on $\Sigma$ and let $\cut$ be a cut value ($0 < \cut \le 1$).Then the \emph{$\cut$-fuzzy derivative} of $\mathcal{L}$ with respect to a symbol $a \in \Sigma$ is such that  $D_a^\cut(\mathcal{L})=\bigcup\{w \mid  [w]\in D_{[a]}(\mathcal L^\mu) \}$,
\end{theorem}

A notion of $\sim_\mu^\Sigma$ can be given also for regular expressions, by just defining inductively on the syntax of regualr expressions the equivalence class $[\ ]^\mu$. All results on fuzzy regular expressions go through w.r.t. the equivalence classes, {\em i.e.} the language of representatives.}
