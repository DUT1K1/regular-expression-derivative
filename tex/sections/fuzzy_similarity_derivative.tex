...
\begin{theorem}Let $\cR$ be a fuzzy similarity relation on $\Sigma$ and let $\cut$ be a cut value ($0 < \cut \le 1$).Then the \emph{$\cut$-fuzzy derivative} of $\mathcal{L}$ with respect to a symbol $a \in \Sigma$ is such that  $D_a^\cut(\mathcal{L})=\bigcup\{[c]\mid D_{[a]}(\mathcal L^\mu) \}$,
\end{theorem}}
