% =====================================================================
%  Beamer presentation — XVI International Conference of the
%  Georgian Mathematical Union (GMU), Batumi, 31 Aug – 5 Sep 2026.
%  Section: Logic, Language, Artificial Intelligence.
%
%  Fuzzifying Derivatives of Regular Expressions
%  Dundua, Honsell, Katamashvili, Maisuradze.
%
%  Adapted from the Women in Logic / FLoC 2026 deck. Changes:
%    * GMU title block (four authors, three affiliations, venue).
%    * New frame "The fuzzy correspondence theorem": the naive fuzzy
%      correspondence is false (counterexample), corrected statement
%      saturates the left side with L^mu and fixes the epsilon base case.
%    * Quotient image renamed [L]_mu, to free L^mu for the fuzzy
%      interpretation; the WiL deck used L^mu for both.
%
%  Build:
%      latexmk -pdf presentation.tex
%  or  pdflatex presentation.tex   (run twice, for the outline/toc)
%
...
\subtitle{XVI International Conference of the\\Georgian Mathematical Union\\[0.3em]{\small Section: Logic, Language, Artificial Intelligence}}
\author[Dundua, Honsell, Katamashvili, Maisuradze]{%
  Besik Dundua\inst{1,2} \and
  Furio Honsell\inst{3} \and
  \underline{Mariam Katamashvili}\inst{1,2} \and
  David Maisuradze\inst{1}}
\institute[]{%
  \inst{1}Kutaisi International University, Kutaisi, Georgia \\
  \inst{2}VIAM, Tbilisi State University, Tbilisi, Georgia \\
  \inst{3}University of Udine, Udine, Italy}
\date[Batumi 2026]{Batumi, 31 August \textendash\ 5 September 2026}
...
\begin{frame}{Fuzzy derivatives}
  ...
  \begin{definition}[Fuzzy expression derivative]
    ...
    \]
    a \emph{finite} sum of ordinary derivatives: for finite $\Sigma$ only finitely
    many $s$ have $\cR^E(s,r)\neq0$.
  \end{definition}
...
\begin{frame}{G\"odel's min: fuzzy = crisp over a quotient}
  ...
  \begin{definition}[Quotient alphabet]
    ...
    $q_\cut(a)=[a]_\cut$, extended to words by $q_\cut^*$ and to restricted
    expressions by $q_\cut^*(r[a_1,\dots,a_n])=r[[a_1]_\cut,\dots,[a_n]_\cut]$.
    The \emph{quotient image} of a language is
    $[L]_\cut=q_\cut^*(L)\subseteq(\Sigma^\cut)^*$.
  \end{definition}
  \begin{theorem}[Reduction to the quotient alphabet]
    ...
    \[
      \partial_a^\cut(L)=\{\,w\mid q_\cut^*(w)\in\partial_{[a]_\cut}([L]_\cut)\,\},
      \qquad
      \big[\partial_a^\cut(L)\big]_\cut=\partial_{[a]_\cut}\big([L]_\cut\big).
    \]
  \end{theorem}
  {\footnotesize Do not confuse the two: $[L]_\cut$ is the quotient \emph{image},
  a language over $\Sigma^\cut$; $\mathcal L^\cut(r)$ on the next slides is the
  \emph{fuzzy interpretation}, a language over $\Sigma$.}
\end{frame}

\begin{frame}{The fuzzy correspondence theorem}
  Crisp: $\mathcal L(\Dd_u(r))=\partial_u(\mathcal L(r))$. The naive fuzzy copy
  $\mathcal L(\Dd_a^\cut(r))=\partial_a^\cut(\mathcal L(r))$ is \alert{false}.
  \begin{block}{Counterexample}
    $\Sigma=\{a,b\}$, $\cR(a,b)=\cut<1$, $r=a\st$. Both $a\st$ and $b\st$ are
    $\cut$-similar to $r$, so
    $\Dd_a^\cut(a\st)=\Dd_a(a\st)+\Dd_b(a\st)+\Dd_a(b\st)+\Dd_b(b\st)\sim a\st{+}b\st$.

    Now $ab\in\partial_a^\cut(\mathcal L(a\st))$: take $b'=a$ and $v=aa$, since
    $\cR(a,a)=1$, $\cR^w(ab,aa)=\min\{1,\cR(b,a)\}=\cut$ and $aaa\in a\st$.
    But $ab\notin\mathcal L(a\st{+}b\st)=\mathcal L(\Dd_a^\cut(a\st))$, so
    \[
      \mathcal L\bigl(\Dd_a^\cut(a\st)\bigr)\;\alert{\neq}\;\partial_a^\cut\bigl(\mathcal L(a\st)\bigr).
    \]
  \end{block}
  \vfill
  \begin{block}{The corrected theorem}
    \[
      \boxed{\;\mathcal L^\cut\bigl(\Dd_u^\cut(r)\bigr)=\partial_u^\cut\bigl(\mathcal L(r)\bigr)\;}
      \qquad\text{with}\quad \partial_\eps^\cut(\mathcal L):=\mathcal L^\cut .
    \]
    In the counterexample above, the corrected left-hand side gives
    $\mathcal L^\cut(a\st{+}b\st)=\Sigma^*$, which is exactly
    $\partial_a^\cut(\mathcal L(a\st))$.
  \end{block}
\end{frame}

\begin{frame}{Everything carries over}
  Because the fuzzy theory is just the crisp theory read over $\Sigma^\cut$,
  each earlier result transfers, once both sides are saturated.
  \begin{itemize}
    \item \textbf{Fuzzy interpretation:}
      $\ \mathcal L^\cut(r)=\{\,w\mid w\sim_\cut^{\Sigma^*}v\ \text{for some}\ v\in \mathcal L(r)\,\}
       =(q_\cut^*)^{-1}\bigl([\mathcal L(r)]_\cut\bigr)$.
    \item \textbf{Correspondence:}
      $\ \mathcal L^\cut\bigl(\Dd_u^\cut(r)\bigr)=\partial_u^\cut\bigl(\mathcal L(r)\bigr)$,
      \ with the base case $\partial_\eps^\cut(\mathcal L):=\mathcal L^\cut$.
    \item \textbf{Finiteness:} $\ \{\partial_u^\cut(\mathcal L)\mid u\in\Sigma^*\}$ is finite.
    \item \textbf{Fuzzy automaton:} $\ \mathcal L(M^\cut)=\mathcal L^\cut(r)$,
      the words $\cut$-close to some word of $r$.
  \end{itemize}
  \vfill
  Over the original alphabet $\Sigma$, the quotient transition $\delta(q_1,[a]_\cut)=q_2$
  fires on every symbol $\cut$-similar to $a$, and is therefore a
  \textbf{fuzzy transition} of degree at least $\cut$.
\end{frame}
...
\begin{frame}{Conclusion}
  ...
  \begin{itemize}
    ...
    \item The \textbf{quotient alphabet} $\Sigma^\cut$ reduces the fuzzy theory
          to the crisp one, so every earlier result carries over.
  \end{itemize}
  \medskip
  \textbf{Future work.}
  \begin{itemize}
    \item Drop G\"odel's $\min$ for a \textbf{general left-continuous T-norm}.
    \item Formalize the \textbf{fuzzy layer} in Rocq.
    \item Obtain a constructive \textbf{Kuratowski-finiteness} result via
